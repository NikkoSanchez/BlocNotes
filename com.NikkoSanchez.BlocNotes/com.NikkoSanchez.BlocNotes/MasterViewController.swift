...
class MasterViewController: UITableViewController, NSFetchedResultsControllerDelegate, UISearchBarDelegate, UISearchControllerDelegate, UISearchResultsUpdating {
    ...
    func requestiCloudPermission(){
        ...
        let firstLaunchWithiCloudAvailable = defaults.bool(forKey: firstLaunchKey)
        let currentiCloudToken = FileManager.default.ubiquityIdentityToken
        if (currentiCloudToken == nil) && !firstLaunchWithiCloudAvailable {
            let alertView = UIAlertController(title: "Choose Storage Option", message: "Should documents be stored in iCloud and available on all your devices?", preferredStyle: UIAlertControllerStyle.alert)
            let cancelAction = UIAlertAction(title: "Local Only", style: UIAlertActionStyle.cancel, handler: {
                (alert) in
                //guard let firstLaunchKey = self.firstLaunchKey else { return }
                defaults.set(false, forKey: firstLaunchKey)
            })
            
            let otherAction = UIAlertAction(title: "Use iCloud", style: UIAlertActionStyle.default, handler: {      (alert) in
                //guard let firstLaunchKey = self?.firstLaunchKey else { return }
                defaults.set(true, forKey: firstLaunchKey)
                guard let appDelegate = UIApplication.shared.delegate as? AppDelegate else {return}
                appDelegate.setUpiCloudAsync()
            })
